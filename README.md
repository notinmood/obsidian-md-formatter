...
- **格式化选中内容** - 仅格式化选中的文本片段
- **批量格式化文件夹** - 文件夹选择对话框 + 右键菜单，可选择任意文件夹批量处理所有 Markdown 文件
  - 格式化前确认对话框（显示待处理文件数量）
  - 实时进度显示（进度条、当前文件名、成功/失败计数）
  - 智能排除特殊文件夹（`.obsidian`、`node_modules` 等）
...
- **AI 格式化标记** - 已 AI 格式化的文档自动标记 `ai-formatted` 时间戳，避免重复调用 AI 节省用量
...
| 标签 (tags) | 确保时间标签（Year/YYYY、Month/MM）存在且排在最前；AI 生成内容标签 | 确定性逻辑否，AI 标签是 |
| 摘要 (summary) | AI 生成摘要（已有摘要不覆盖） | 是 |
| 分类 (categories) | AI 生成分类 | 是 |
| AI 格式化标记 (ai-formatted) | AI 格式化后写入时间戳，已有值时跳过 AI 调用 | 是（标记） |
...
**规则配置**
- Frontmatter 格式化 - 主开关 + 子规则折叠面板（字段规范化、created、updated、ai-formatted、tags、summary、categories、title 各自独立配置）
...
### AI 格式化标记 (ai-formatted)

为避免重复调用 AI 浪费用量，插件会在 AI 格式化后自动在 frontmatter 中写入 `ai-formatted` 字段（值为格式化时间）：

- **已有 `ai-formatted` 时间值** → 下次格式化跳过 AI 调用，仅执行确定性逻辑
- **无此字段或值为空** → 正常调用 AI 并写入新的时间戳
- **强制重新调用 AI** → 删除 `ai-formatted` 字段或清空其值，下次格式化即可重新触发

可在设置面板中关闭此行为：
- 关闭"启用 ai-formatted 字段"→ 不写入该字段
- 关闭"已有值时跳过 AI 调用"→ 每次都调用 AI
